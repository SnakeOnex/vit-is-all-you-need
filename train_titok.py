...
if __name__ == '__main__':
    ...
    for epoch in range(10000):
        ...
        for i, (images, labels) in enumerate(bar):
            ...
            # print(f"{latent_embs.shape=}")
            latent_embs_titok = rearrange(latent_embs, 'b c w -> b c 1 w')
            quantized, result_dict = quantizer_titok(latent_embs_titok)
            quantize_loss = result_dict["quantizer_loss"]
            # print(f"{quantized.shape=}")
            quantized = rearrange(quantized, 'b c h 1 -> b h c')
